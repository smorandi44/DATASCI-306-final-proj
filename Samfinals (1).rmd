...
Estimates <- read_excel("data/WPP2024_GEN_F01_DEMOGRAPHIC_INDICATORS_COMPACT.xlsx", 
    sheet = "Estimates", skip = 15)
Medium_variant <- read_excel("data/WPP2024_GEN_F01_DEMOGRAPHIC_INDICATORS_COMPACT.xlsx",     sheet = "Medium variant", skip = 15)
```
...
```{r}
# this code made it so that the top row of the data frame became the column names
Estimates <- Estimates |>
  setNames(Estimates[1, ]) |>
  slice(-1)

Medium_variant <- Medium_variant |>
  setNames(Medium_variant[1, ]) |>
  slice(-1)
